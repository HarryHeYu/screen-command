...
2. **框选**：按 `Ctrl+Shift+A`（或点状态窗口的 Capture 按钮），屏幕变暗并冻结当前画面；按住鼠标左键拖出一个矩形，松开即完成框选。拖拽过程中选区会恢复亮度并显示实际像素尺寸。按 `Esc` 或单击（不拖动）可随时取消。
...
5. **快捷路径**：`Ctrl+Shift+1` 跳过菜单——框选后自动 OCR 并把结果复制到剪贴板，适合高频的"复制屏幕文字"场景。
